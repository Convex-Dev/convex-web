...
(deftest datafy-test
  (let [context (make-convex-context)]
    ...
    (testing "Byte"
             (is (= convex.core.data.prim.CVMByte
                    (type (convex/execute-string context "(nth 0xFF 0)"))))
             (is (= 255
                    (convex/datafy (convex/execute-string context
                                                          "(nth 0xFF 0)")))))

    (testing "Char" (is (= \n (convex/datafy (convex/execute context \n)))))

    (testing "Long" (is (= 1 (convex/datafy (convex/execute context 1)))))

    (testing "Double" (is (= 1.0 (convex/datafy (convex/execute context 1.0)))))

    (testing "Keyword" (is (= :a (convex/datafy (convex/execute context :a)))))
    ...
    (testing "Vector" (is (= [] (convex/datafy (convex/execute context [])))))
    ...
    (testing "Set" (is (= #{} (convex/datafy (convex/execute context #{})))))
